    ...
    <svg viewBox="0 0 460 300" className="w-full h-auto" role="img"
      ...
      <defs>
        <pattern id="plateHatch" width="6" height="6" patternUnits="userSpaceOnUse"
          ...
          <line x1="0" y1="0" x2="0" y2="6" stroke="#98999B" strokeWidth="1.4" />
        </pattern>
      </defs>

      {/* ══ Titles ══ */}
      <text x={L_X + 48} y="18" textAnchor="middle" fontSize="11" fontWeight="600" fill="#4B7B4E">
        ✓ ~45° from vertical
      </text>
      <text x={L_X + 48} y="31" textAnchor="middle" fontSize="9" fill="#5F6164">
        each layer lands on the one below
      </text>
      <text x={R_X + 60} y="18" textAnchor="middle" fontSize="11" fontWeight="600" fill="#9B3B3E">
        ✗ steeper than ~45°
      </text>
      <text x={R_X + 60} y="31" textAnchor="middle" fontSize="9" fill="#5F6164">
      ...
      {wallLayers.map(y => (
        <rect key={`lw-${y}`} x={L_X} y={y} width={COL_W} height={LH}
              fill="#BFC9D8" stroke="#2B4C7E" strokeWidth="1" />
      ))}
      ...
      <line x1={L_X + COL_W} y1={COL_TOP} x2={L_X + COL_W} y2={COL_TOP - 68}
            stroke="#1E3659" strokeWidth="1" strokeDasharray="3 3" />
      <line x1={L_X + COL_W} y1={COL_TOP} x2={lRight(L_COUNT - 1)} y2={COL_TOP - LH * L_COUNT}
            stroke="#1E3659" strokeWidth="1.2" strokeDasharray="4 2" />
      <path d={`M${L_X + COL_W},${COL_TOP - 46} A 46 46 0 0 1 ${L_X + COL_W + 32.5},${COL_TOP - 32.5}`}
            fill="none" stroke="#1E3659" strokeWidth="1.2" />
      <text x={L_X + COL_W + 6} y={COL_TOP - 52} fontSize="10" fontWeight="700" fill="#1E3659">
        45°
      </text>

      <text x={L_X - 8} y={COL_TOP - LH * L_COUNT - 16} fontSize="9" fill="#4B7B4E" fontWeight="600">
        bonds normally
      </text>
      ...
      {wallLayers.map(y => (
        <rect key={`rw-${y}`} x={R_X} y={y} width={COL_W} height={LH}
              fill="#F3E7E8" stroke="#9B3B3E" strokeWidth="1" />
      ))}
      ...
      */}
        ...
        return (
          <path
            ...
            d={`M${R_X},${y}
                ...
                L${R_X},${y + LH} Z`}
            fill="#F3E7E8" stroke="#9B3B3E" strokeWidth="1" strokeLinejoin="round"
          />
        );
      })}
      ...
      <line x1={R_X + COL_W} y1={COL_TOP} x2={R_X + COL_W} y2={COL_TOP - 58}
            stroke="#7A2B2E" strokeWidth="1" strokeDasharray="3 3" />
      <line x1={R_X + COL_W} y1={COL_TOP} x2={rRight(R_COUNT - 1)} y2={COL_TOP - LH * R_COUNT}
            stroke="#7A2B2E" strokeWidth="1.2" strokeDasharray="4 2" />
      <path d={`M${R_X + COL_W},${COL_TOP - 38} A 38 38 0 0 1 ${R_X + COL_W + 33.5},${COL_TOP - 20.5}`}
            fill="none" stroke="#7A2B2E" strokeWidth="1.2" />
      <text x={R_X + COL_W + 4} y={COL_TOP - 44} fontSize="10" fontWeight="700" fill="#7A2B2E">
        ~62°
      </text>
      ...
      <text x={rRight(R_COUNT - 1) - 34} y={COL_TOP - LH * R_COUNT - 12}
            fontSize="9" fontWeight="600" fill="#9B3B3E">
        sags before it sets
      </text>

      {/* ══ Build plate ══ */}
      <rect x="20" y={PLATE_Y} width="420" height="13" fill="#E7EAEC"
            stroke="#5F6164" strokeWidth="1.4" />
      <rect x="20" y={PLATE_Y} width="420" height="13" fill="url(#plateHatch)" stroke="none" />
      <text x="230" y={PLATE_Y + 25} textAnchor="middle" fontSize="9" fill="#5F6164">
        build plate
      </text>

      {/* ══ Footnote ══ */}
      <text x="230" y="282" textAnchor="middle" fontSize="9" fill="#98999B" fontStyle="italic">
