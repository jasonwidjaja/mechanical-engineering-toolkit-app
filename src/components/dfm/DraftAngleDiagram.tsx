...
export default function DraftAngleDiagram() {
  ...
  return (
    <svg viewBox="0 0 460 300" className="w-full h-auto" role="img"
      ...
      {/* ══ Titles ══ */}
      <text x={L_CX} y="18" textAnchor="middle" fontSize="11" fontWeight="600" fill="#16a34a">
        ✓ 1–2° draft — releases cleanly
      </text>
      <text x={R_CX} y="18" textAnchor="middle" fontSize="11" fontWeight="600" fill="#dc2626">
      ...
      {/* Ejection direction */}
      <line x1={L_CX} y1="80" x2={L_CX} y2="48" stroke="#475569" strokeWidth="1.6"
            markerEnd="url(#draftArrow)" />
      <text x={L_CX + 8} y="62" fontSize="9" fill="#64748b">ejection</text>
      ...
      <path
        ...
        fill="#bfdbfe" stroke="#2563eb" strokeWidth="1.8" />
      ...
      <line x1={Lw_topL} y1={Lp_topY} x2={Lw_topL} y2={Lp_topY + 34}
            stroke="#2563eb" strokeWidth="1" strokeDasharray="3 3" opacity="0.8" />
      ...
      <path d={`M${Lw_topL},${Lp_topY + 28} A 28 28 0 0 0 ${Lw_topL + 5.2},${Lp_topY + 27.5}`}
            fill="none" stroke="#2563eb" strokeWidth="1.3" />
      ...
      <text x="4" y={Lp_topY - 10} fontSize="10" fontWeight="600" fill="#2563eb">
        draft angle
      </text>

      {/* Call out the gap that draft buys you */}
      <text x={Lw_botR + 16} y={Lp_botY + 4} fontSize="9" fill="#16a34a" fontWeight="600">
        gap opens
      </text>
      <text x={Lw_botR + 16} y={Lp_botY + 14} fontSize="9" fill="#16a34a">
        immediately
      </text>

      {/* ══════════════════ RIGHT — UNDRAFTED ══════════════════ */}

      <line x1={R_CX} y1="80" x2={R_CX} y2="48" stroke="#475569" strokeWidth="1.6"
            markerEnd="url(#draftArrow)" />
      <text x={R_CX + 8} y="62" fontSize="9" fill="#64748b">ejection</text>
      ...
      {[128, 140, 152, 164, 176].map(y => (
        <g key={y}>
          <path d={`M${Rw_L - 4},${y} l8,4`} stroke="#dc2626" strokeWidth="1.4" />
          <path d={`M${Rw_R - 4},${y} l8,-4`} stroke="#dc2626" strokeWidth="1.4" />
        </g>
      ))}

      <text x={Rw_R + 14} y={Rp_botY - 2} fontSize="9" fill="#dc2626" fontWeight="600">
        full-height
      </text>
      <text x={Rw_R + 14} y={Rp_botY + 8} fontSize="9" fill="#dc2626">
        contact
      </text>

      {/* ══ Footnote ══ */}
      <text x="230" y="256" textAnchor="middle" fontSize="9" fill="#94a3b8" fontStyle="italic">
        Draft exaggerated for clarity — real draft is typically 1–2° per side.
      </text>
      <text x="230" y="270" textAnchor="middle" fontSize="9" fill="#94a3b8">
